...
class Wallet extends CI_Controller
{
	...
	public function walletTransactionHistory()
	{
	    ...
	    if((trim($request['apiusername']) == API_USERNAME) && (trim($request['apipassword']) == API_PASSWORD) && (trim($request['appversion'])) == APP_VERSION)
	    {
	        ...
	        {
    	        $table = USER_WALLET_PAYMENT;
                $getFilterList = $this->Admin_model->getDataArrayField($table, $where=array('user_id'=>trim($request['user_id'])), $orderKey='id', 'id, user_id,amount,transaction_id,transaction_type,transaction_status,created_date');
