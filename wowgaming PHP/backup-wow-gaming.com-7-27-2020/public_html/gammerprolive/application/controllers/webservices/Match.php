	        ...
	        if((trim($request['game_id'])) != "" && (trim($request['filter_id'])) != "" && (trim($request['user_id'])) != "")
	        {
    	        ...
    	        if($getMatchList)
    	        {
    	            ...
    	            foreach($getMatchList as $match)
    	            {
    	                ...
    	                {
    	                    $getTeam = $this->Admin_model->getDataArrayField(CREATE_TEAM_PLAYER, $where=array('game_id'=>trim($request['game_id']),'match_id'=>$match['id'],'user_id'=>trim($request['user_id']),'assign_with_other_team'=>'0','active_status'=>'1'), $orderKey='team_id', $fields='team_id');
    	                    $matchData[$i]['play_status'] = '0';
    	                    $matchData[$i]['pair_id'] = '';
    	                    if($getTeam)
    	                    {
    	                        $matchData[$i]['team_id'] = $getTeam[0]['team_id'];
    	                    }else
    	                    {
    	                        $matchData[$i]['team_id'] = '';
    	                    }
